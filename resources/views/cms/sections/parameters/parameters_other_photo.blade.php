...
<div class="card card-preview">
    <div class="card-inner">
        <table class="nk-tb-list nk-tb-ulist font-helvetica-regular">
            ...
            <tbody>
                @foreach($other_photo_list as $photo_item)
                <tr class="nk-tb-item">
                    <td class="nk-tb-col">
                        <span>{{ $photo_item->name }}</span>
                    </td>
                    <td class="nk-tb-col">
                        <div class="user-card">
                            <div class="user-info">
                                <img src="{{ url('uploads/other/'.$photo_item->photo) }}">
                            </div>
                        </div>
                    </td>
                    <td class="nk-tb-col text-right">
                        <div class="form-group">
                            <span onclick="UpdateOtherPhoto({{ $photo_item->id }})">სურათის განახლება</span>
                        </div>
                    </td>
                </tr>
                @endforeach
                ...
                <form action="#" class="form-validate is-alter" id="photo_form">
                    <div class="row">
                        ...
                        <div class="col-lg-12 mb-3">
                            <div class="form-group">
                                <button type="button" onclick="UpdateOtherPhotoSubmit()" class="btn btn-lg btn-primary font-helvetica-regular">შენახვა</button>
                            </div>
                        </div>
                    </div>
                </form>
...
<script type="text/javascript">
    function UpdateOtherPhoto(photo_id) {
        $.ajax({
            dataType: 'json',
            url: "/cms/ajax/ajaxUpdateOtherPhoto",
            type: "GET",
            data: {
                photo_id: photo_id,
            },
    ...
    function UpdateOtherPhotoSubmit() {
        var form = $('#photo_form')[0];
        var data = new FormData(form);

        $.ajax({
            dataType: 'json',
            url: "/cms/ajax/ajaxUpdateOtherPhotoSubmit",
            type: "POST",
            data: data,
            processData: false,
            contentType: false,
